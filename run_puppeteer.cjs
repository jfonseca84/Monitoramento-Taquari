...
(async () => {
  ...
  try {
    await page.evaluate(() => {
      ...
      const centroBtn = btns.find(b => b.textContent.includes('CENTRO DE ANÁLISES'));
      if (centroBtn) {
         console.log('Clicking CENTRO DE ANÁLISES');
         centroBtn.click();
      } else {
         console.log('Could not find CENTRO DE ANÁLISES tab');
      }
    });
