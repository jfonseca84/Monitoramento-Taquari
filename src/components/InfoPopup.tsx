...
export const InfoPopup: React.FC<InfoPopupProps> = ({ open, onClose, label, children }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-3 top-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] rounded-2xl bg-[var(--map-bg)] text-[var(--map-text)] shadow-2xl overflow-hidden">
        <div className="h-full overflow-y-auto overscroll-contain px-6 pt-8 pb-28">{children}</div>
