...
export const CITY_THRESHOLDS: Record<string, { normal: number; attention: number; alert: number; flood: number }> = {
  'Santa Tereza': { normal: 4.0, attention: 6.0, alert: 9.0, flood: 15.0 },
  'Muçum': { normal: 4.0, attention: 5.0, alert: 9.0, flood: 18.0 },
  'Encantado': { normal: 4.0, attention: 5.0, alert: 9.0, flood: 12.0 },
  ...
  'Montenegro': { normal: 2.0, attention: 3.0, alert: 4.0, flood: 6.0 },
  ...
  'Rio Pardo': { normal: 7.0, attention: 10.0, alert: 11.5, flood: 12.5 },
};
...
export async function fetchFromANA(catalogCities: DBCity[] = []): Promise<RawStationPayload[]> {
  ...
  return results.filter((r): r is RawStationPayload => r !== null);
}
      ...
      for (const rawPayload of rawPayloads) {
        try {
          ...
          if (typeof stPayload.rate === 'number' && !isNaN(stPayload.rate)) {
            // Se a taxa for informada em cm/h (ex: -3.5 ou 3.5), converte para m/h (-0.035)
